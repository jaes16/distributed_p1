...
int client_service(int new_socket) {
    ...
    while(1){
        ...
        // reset timeout
        sem_wait(&sem);
        tv.tv_sec = MAX_TIMEOUT - ((*num_processes) * 10);
...
int main() {
    ...
    num_processes = mmap(0,                         // set addr to 0 so that (w/o MAP_FIXED flag) the system will choose a free spot 
                        sizeof(int),                
                        PROT_READ | PROT_WRITE,     // set permissions
                        MAP_ANON | MAP_SHARED,      // put in anonymous region, so that only this process and children can change
                                                    // also, allow modifications to be shared
                        -1,                         // fd is -1 so that it isn't written out to persistent storage
                        0
                    );
    sem_init(&sem, 1, 0); // initialize semaphore, allow processes to interact
    while(1){
        sem_wait(&sem);
        if (*num_processes < MAX_PROCESSES){
            sem_post(&sem); // because other processes can only decrease the count
